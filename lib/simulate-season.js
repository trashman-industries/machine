...
const simulateSeason = async ({ current_week, leagueId, teams, standings, schedule }) => {
  ...
  for (let i=0; i<number_of_simulations; i++) {
    ...
    simulation_standings = simulation_standings.sort((a, b) => {
      const a_wins = a.projected_wins
      const b_wins = b.projected_wins
      ...
      const a_points = team_a.projected_points
      const b_points = team_b.projected_points
    ...
    // championship
    const champion = simulatePlayoffGame(game_three_winner, game_four_winner, 16)
  ...
  copied_standings = copied_standings.sort((a, b) => {
    const p = b.simulated_playoff_appearances - a.simulated_playoff_appearances
    if (p !== 0)
      return p

    return b.simulated_championship_win - a.simulated_championship_win
  })
  ...
  for (const team of copied_standings) {
    ...
    team.total_points = team.projected_points.toFixed(1)
